...
)
{
    ...
    switch (parcelBasis_)
    {
        case pbMass:
        {
            ...
            //scalar volumeTot = massTotal_/rho; //ORIGINAL
            //nP = (volumeFraction*volumeTot + delayedVolume_)/(parcels*volumep); //ORIGINAL
	    nP = floor(Vdispon/((parcels-iter)*volumep));
	    //nP = floor(Vdispon/((parcels)*volumep)); //ALTERNATIVE
            break;
        }
        case pbNumber:
        {
            nP = massTotal_/(density*volumeTotal_);
            break;
        }
        case pbFixed:
        {
...
)
{
    ...
    const scalar allParcelsDeleted = returnReduce(deleted, sumOp<scalar>());
        ...
        Info<< nl
            ...
            << "    Added " << allParcelsAdded << " new parcels of " << newParcels << nl
            << "    Deleted " << allParcelsDeleted << " parcels because parcel mass too low" << nl
...
void Foam::InjectionModel<CloudType>::inject(TrackData& td)
{
	...
	scalar Vdisp = delayedVolume_ + newVolumeFraction*massTotal_/density; //instead of scalar delayedVolume = 0;

        // Duration of injection period during this timestep
        const scalar deltaT = max(0.0, min(trackTime, min(time - SOI_, timeEnd() - time0_)));
        ...
        for (label parcelI = 0; parcelI < newParcels; parcelI++)
        {
            if (validInjection(parcelI))
            {
                ...
                // Determine the injection position and owner cell, tetFace and tetPt
                label cellI = -1;
                label tetFaceI = -1;
                label tetPtI = -1;
    ...
    postInjectCheck(parcelsAdded, massAdded, deleted, newParcels, newVolumeFraction);
}
...
)
{
    ...
    //Set density equal to rho0
    scalar density = cloud.constProps().rho0();
	...
	scalar Vdisp = newVolumeFraction*massTotal_/density;
